...
class TestArduinoUno(unittest.TestCase):
    ...
    def test_analog_write_sets_state(self):
        self.device.analog_write(3, 100)
        expected = (31, 3, 100)
        self.assertEqual(self.device.commands.next_command(), expected)

    def test_analog_write_with_incorrect_state(self):
        with self.assertRaises(ValueError):
            self.device.analog_write(3, 500)
            self.device.analog_write(3, -1)
        with self.assertRaises(TypeError):
            self.device.analog_write(3, 10.1)
    ...
    def test_analog_write_on_non_pwm_pin_raises_error(self):
        with self.assertRaisesRegexp(PinError, 'cannot complete operation as pwm=False for pin 0'):
