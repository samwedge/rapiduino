...
from rapiduino.commands import CMD_DIGITALWRITE, CMD_PARROT, CMD_VERSION
...
class TestSerialConnection(unittest.TestCase):
    ...
    def test_open_with_valid_connection_parameters(self) -> None:
        with patch("rapiduino.communication.Serial", autospec=True) as mock_serial:
            ...
            self.assertEqual(
                mock_serial.call_args, call(self.port, baudrate=115200, timeout=5)
            )

    def test_open_with_error(self) -> None:
        with patch("rapiduino.communication.Serial", autospec=True) as mock_serial:
    ...
    def test_close_when_connection_open(self) -> None:
        self.serial_connection.close()
        self.mock_conn.close.assert_called_once_with()

    def test_process_command_with_valid_command_returning_bytes(self) -> None:
    ...
    def test_process_command_with_valid_command_returning_zero_bytes(self) -> None:
